  ...
  return (
    <div className="grid rounded-2xl bg-surface py-3.5" style={{ gridTemplateColumns: `repeat(${items.length}, 1fr)` }}>
      {items.map((item, i) => (
        ...
        >
          <p className="font-display text-[22px] leading-none text-accent">
            <span className="tabular-nums">{item.value}</span>
            {item.unit ? <span className="ml-0.5 text-[13px]">{item.unit}</span> : null}
          </p>
          <p className="mt-1.5 text-[10.5px] text-ink-soft">{item.label}</p>
        </div>
      ))}
    </div>
  );
