  ...
  return (
    <div className="mx-auto flex h-full max-w-[480px] flex-col bg-bg px-7">
      <main className="flex min-h-0 flex-1 flex-col items-center justify-center">
        ...
        <h1 className="mt-7 font-display text-3xl">젤리젤리</h1>
        <p className="mt-2 text-center text-base leading-relaxed text-ink-soft">
        ...
        >
          <input
            ...
            enterKeyHint="done"
            maxLength={12}
            className="w-full rounded-2xl bg-surface px-4 py-3 text-center text-md outline-none placeholder:text-ink-faint focus:shadow-[inset_0_0_0_1.5px_var(--accent)]"
          />
          <button
            type="submit"
            disabled={!ready || busy}
            className="mt-2.5 w-full rounded-2xl bg-accent py-3.5 font-display text-lg text-white transition active:scale-[.98] disabled:opacity-35"
          >
            시작하기
          </button>
        </form>

        <p className="mt-3 text-tiny text-ink-faint">나중에 설정에서 바꿀 수 있어요</p>
      </main>
    </div>
  );
}
