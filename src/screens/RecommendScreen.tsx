          ...
          warmedUp ? (
            <>
              ...
              {view.again.length + view.close.length + view.newLand.length === 0 ? (
                <p className="mt-10 text-center text-[13px] leading-relaxed text-ink-soft">
                  도감을 거의 다 드셨네요.
                  <br />＋ 로 새 젤리를 추가해 보세요.
                </p>
              ) : null}
            ...
            <>
              <section className="rounded-2xl bg-surface p-4">
                <p className="text-[13px] font-medium">아직 추천할 만큼은 아니에요</p>
...
function TasteCard({ taste }: { taste: TasteProfile }) {
  ...
  return (
    <section className="rounded-2xl bg-surface p-4">
      <p className="text-[11px] text-ink-soft">기록 {taste.eaten}개로 본 내 취향</p>
      ...
      {chips.length > 0 ? (
        <div className="mt-3 flex flex-wrap gap-1.5">
          {chips.map((chip) => (
            <span
              key={chip}
              className="rounded-full bg-accent-bg px-2.5 py-1 text-[10.5px] font-medium text-accent"
            >
              {chip}
            </span>
          ))}
        ...
        {items.map(({ jelly, reason }) => (
          <li key={jelly.id}>
            ...
            >
              ...
              <span className="flex-none text-[16px] text-ink-faint">›</span>
            </Link>
          </li>
        ))}
      </ul>
