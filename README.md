...
**기울기 허락은 눈에 보이는 버튼이어야 한다.** 처음엔 병을 톡 치는 동작에
얹었는데, 알을 굵게 만든 뒤로는 병을 눌러도 젤리가 먼저 받아버려서 흔들기가
안 불리고, 따라서 물음도 영영 안 떴다. 숨은 동작에 얹으면 그 동작이 막히는
날 기능이 통째로 사라진다. 그래서 '폰 기울이면 쏠리게 하기' 버튼을 따로 뒀다.
손가락이 닿는 기기에서만, 아직 안 켜졌을 때만 보인다.
