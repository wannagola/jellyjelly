...
<tr>
<td><b>도감</b> — 편의점 젤리 73종이 기본으로 깔려 있다. 안 먹어본 것은 흐리게. 최애 / 모은 것 / 전체로 나눠 본다.</td>
...
| 도토리 받기 | 가을 놀이. 다람쥐가 바구니를 들고 받는다. 아가리 안에 떨어져야 받은 것. 밤송이는 피한다 |
...
**도감은 계정마다 따로다.** 가입하면 편의점 젤리 73종이 각자에게 깔린다.
...
**씨앗을 깔 때 읽기와 쓰기는 한 트랜잭션이어야 한다.** "없는 것만 넣는다"는
판단을 트랜잭션 밖에서 하고 넣기만 안에서 했더니, 탭이 둘이면 둘 다 "아직
없네"를 보고 각자 넣었다. 씨앗 버전을 올린 날 새 젤리 다섯 종이 네 벌씩
깔린 걸 배포판에서 봤다. 읽기를 안으로 들여 고쳤고, 이미 겹쳐 받은 사람을
위해 한 번 쓸어내는 것도 같이 넣었다. 다만 손댄 흔적이 있는 벌(기록이
달렸거나 사진·최애·찜·메모·칼로리를 넣어둔 것)은 남긴다 - 겹친 걸 치우다
남의 기록을 지우면 고친 것보다 나쁘다.

묘비에는 줄 id 말고 **무엇이었는지도 적는다.** id 는 기기마다 새로 뽑은 난수라,
씨앗 목록을 다시 깔 때 "이건 지웠던 젤리다"를 알아볼 수가 없었다. 도감 젤리를
지워둔 사람은 씨앗이 한 번 업데이트되는 순간 지운 게 통째로 되살아났을 것이다.
씨앗 버전을 올리기 전까지 아무도 밟지 않았을 뿐이다.
...
src/
  ...
  lib/autumn.ts            다람쥐·바구니·도토리·은행잎·밤송이 그리기와 소리
...
행사 목록에서 실제 판매 중인 제품을 확인해 68종을 직접 정리했고, 이후 요청받은
제품을 하나씩 찾아 더해 73종이 됐다. 앱은 실행 중에 외부 API를 한 번도 부르지 않는다.
...
**받는 자리는 그림으로 있어야 한다.** 처음엔 다람쥐가 팔을 벌리고 서 있고,
몸 가운데에서 화면 폭의 14% 안에 들어오면 받은 것으로 쳤다. 그 14%가 어디부터
어디까지인지는 화면 어디에도 안 나와서, 빗맞으면 왜 못 받았는지 알 수가 없다.
바구니를 들려줬다. 이제 받는 자리가 바구니 아가리고, 아가리를 지나갔는지로만
따진다.

그러려면 그리는 쪽과 따지는 쪽이 같은 숫자를 봐야 한다. 바구니 치수를 한 군데
적어두고 양쪽이 그걸 읽는다. 따로 적으면 언젠가 한쪽만 고쳐서, 바구니 밖에
떨어졌는데 받아지거나 그 반대가 된다. 화면 크기 넷에서 그린 테두리와 규칙의
경계가 어긋나는지 재봤다 - 규칙이 보는 선은 테두리 획의 한가운데였다.

**바구니는 몸이 기울어도 같이 안 기운다.** 달릴 때 다람쥐가 기우는데 바구니까지
같이 기울였더니, 힘껏 달리는 순간 그려진 아가리가 규칙이 보는 자리에서 21px 까지
밀렸다. 반폭이 50px 이니 바구니에 든 게 안 받아지는 거리다. 들고 뛰면 수평을
잡는 게 자연스럽기도 하고, 무엇보다 받는 자리가 흔들리면 안 된다. 팔은 바구니를
따라가게 해서 손과 테두리가 안 어긋나게 했다.

받았는지는 구간이 아니라 **선을 지나쳤는지**로 본다. "아가리 높이 근처" 같은
구간으로 보면, 속도가 붙었을 때 한 프레임에 그 구간을 건너뛰어서 바구니를
통과해 버린다.
